...
hw9a <- read_excel("datasets/ClusterRCT.xlsx")
hw9b <- read_excel("datasets/ClusterRCT2.xlsx")
...
```{r}
table <- my.tt_table(32,3,14,11, "(fruc)")

my.logit_to_pp <- function(a) {
  exp(a)/(1+exp(a))
}
my.logit_to_pp(-0.5)
```
...
my.ORfunction(95,322,330,372,301)
```
...
glm.fit = glm(Kyphosis~Age+Number+Start, family = binomial, data = data)
summary(glm.fit)
Cstat(glm.fit)
step.fit = step(glm.fit, direction=c("both"))
summary(step.fit)
Cstat(step.fit)

# overfitting
glm.fit = glm(Kyphosis~Age+Number+Start
              + Age*Number + Age*Start + Number*Start
              + I(Age^2) + I(Number^2) + I(Start^2),
              family="binomial", data=data)
summary(glm.fit)
step.fit = step(glm.fit, direction = c("both"))
summary(step.fit)
roc.curve(step.fit, data, "Kyphosis")
...
statistic = function(data, selection) {
  glm.fit = glm(Kyphosis~Age+Start+Number, data=data[selection,], family="binomial")
  return(coef(glm.fit)[2])
}

# return boot strap errors for the same coefficients
...
#Comparing highly abnormal and abnormal vs normal: same code with different conditions
logits2 = rep(0,10)
for (j in c(1,11,21,31,41,51,61,71,81,91)) {
  positive=negative=0
  for (i in c(j:(j+9))) {
...
clogit.fit = clogit(improved~treated+strata(id), data=data1)
  # clustering: strata(varName) - variable to be stratified on
...
# GEE model
# gee.fit = gee(y~x1+x2+x3, data=data, family=binomial, id=clusterName, corstr="exchangeable")
gee.fit = gee(improved~treated, data=data1, family=binomial, id=id, corstr="exchangeable")
  # id: tells function which column defines the clusters
  # corstr: correlation matrix - "independence", "fixed", "stat_M_dep", "non_stat_M_dep", "exchangeable", "AR-M", & "unstructured"
summary(gee.fit)
```
...
cor(data2) #1 
summary(glm(InsControl~treatment+age+female, data=data1, family=binomial)) #3,4
summary(gee(InsControl~treatment+age+female, data=data1, family=binomial, id=id, corstr="exchangeable"))
